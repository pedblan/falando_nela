...
- Pode rodar em paralelo com `camara/plenario_discursos` e `senado/ccj_notas`, pois usa `raw/camara/ccjc_eventos/` e checkpoint proprio.
- O `run_id` deve ser distinto dos outros notebooks ativos, porque logs e manifests sao indexados por `run_id`.
- Nao rode duas instancias de `camara/ccjc_eventos` com o mesmo `run_id` ao mesmo tempo.
...
- Com `--resume`, o coletor deve pular particoes concluidas pelo mesmo `run_id` e registros de API, status Escriba, HTML bruto e corpus ja presentes no JSONL do mesmo `run_id`.
